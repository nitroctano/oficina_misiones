...
$jsondata[0] = consulta($con,'SELECT * FROM VW_CBBM_OFRENDAS_DET WHERE ID_OFRENDA_ENC = '.$folio);
$jsondata[1] = consulta($con,'SELECT * FROM CBBM_USUARIOS WHERE ID_USUARIO = '.$jsondata[0][0][2]);
$jsondata[2] = consulta($con,'SELECT * FROM CBBM_OFRENDAS_ENC WHERE ID_OFRENDA_ENC = '.$folio);
//$jsondata[3] = consulta($con,'SELECT USU.NOMBRE, DET.MONTO FROM CBBM_USUARIOS USU WHERE USU.ID_USUARIO IN (SELECT ID_MISIONERO FROM CBBM_OFRENDAS_DET DET WHERE DET.ID_OFRENDA_ENC = '.$folio.' AND DET.MISIONERO = "S") AND USU.EXTRANJERO = "S"');


$jsondata[3] = consulta($con,'SELECT USU.IDENT_ADMIN, DET.MONTO FROM CBBM_OFRENDAS_DET DET LEFT JOIN CBBM_USUARIOS USU ON USU.ID_USUARIO = DET.ID_MISIONERO WHERE USU.EXTRANJERO = "S" AND DET.MISIONERO = "S" AND DET.ID_OFRENDA_ENC = '.$folio);

$jsondata[4] = consulta($con,'SELECT USU.IDENT_ADMIN, DET.MONTO FROM CBBM_OFRENDAS_DET DET LEFT JOIN CBBM_USUARIOS USU ON USU.ID_USUARIO = DET.ID_MISIONERO WHERE USU.EXTRANJERO = "N" AND DET.MISIONERO = "S" AND DET.ID_OFRENDA_ENC = '.$folio);

$jsondata[5] = consulta($con,'SELECT PRO.NOMBRE, DET.MONTO FROM CBBM_OFRENDAS_DET DET LEFT JOIN CBBM_PROYECTOS PRO ON PRO.ID_PROYECTO = DET.ID_MISIONERO WHERE DET.MISIONERO = "N" AND DET.ID_OFRENDA_ENC = '.$folio);
